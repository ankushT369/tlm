...
static char* queryBuilderCreate(struct tokens* arr, int count) {
    ...
    size_t size = strlen("CREATE TABLE  (id INTEGER PRIMARY KEY, link TEXT, title TEXT);")
                  + strlen(table_name) + 1;
...
static char* queryBuilderAdd(struct tokens* arr, int count) {
    ...
    size_t size =
        strlen("INSERT INTO  (link, title) VALUES ('', '');") +
    ...
    sprintf(query,
            "INSERT INTO %s (link, title) VALUES ('%s', '%s');",
            table, link, title);
...
void tokenize(char* str) {
    ...
    for (int i = 0; i < token_count; i++) {
        ...
        // str_pos += token_array[i].len + 1;  // Move to next token position (+1 for space)
        str_pos++;
    }
}
...
static void debug_print() {
    ...
    for (int i = 0; i < token_count; i++) {
        printf("Token %d: \"%s\" (length: %u)\n", 
               i, token_array[i].token, token_array[i].len);
    }
}
